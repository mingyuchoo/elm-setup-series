...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {S: fragment, V: host, _: path, ab: port_, ae: protocol, af: query};
	});
...
var $author$project$Main$PostsReceived = function (a) {
	return {$: 1, a: a};
};
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{av: false, ax: r.ax, aB: r.aB, T: r.T, aH: r.aH, aT: r.aT, ap: r.ap, aV: r.aV}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{ax: $elm$http$Http$emptyBody, aB: r.aB, T: _List_Nil, aH: 'GET', aT: $elm$core$Maybe$Nothing, ap: $elm$core$Maybe$Nothing, aV: r.aV});
};
var $elm$json$Json$Decode$list = _Json_decodeList;
var $author$project$Main$Post = F4(
	function (id, title, authorName, authorUrl) {
		return {M: authorName, N: authorUrl, W: id, ao: title};
	});
...
var $author$project$Main$fetchPosts = $elm$http$Http$get(
	{
		aB: A2(
			...
			A2($elm$core$Basics$composeR, $krisajenkins$remotedata$RemoteData$fromResult, $author$project$Main$PostsReceived),
			$elm$json$Json$Decode$list($author$project$Main$postDecoder)),
		aV: 'http://localhost:8081/posts'
	});
	...
	function (msg, model) {
		if (!msg.$) {
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{A: $krisajenkins$remotedata$RemoteData$Loading}),
				$author$project$Main$fetchPosts);
		} else {
			var response = msg.a;
			return _Utils_Tuple2(
...
var $author$project$Main$FetchPosts = {$: 0};
...
var $author$project$Main$buildErrorMessage = function (httpError) {
	switch (httpError.$) {
		case 0:
			...
			return message;
		case 1:
			return 'Server is taking to o long to respond. Please try again later.';
		case 2:
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						$elm$core$String$fromInt(post.W))
					])),
				A2(
				$elm$html$Html$td,
				_List_Nil,
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$href(post.N)
							]),
						_List_fromArray(
							[
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Events$onClick($author$project$Main$FetchPosts)
					]),
				_List_fromArray(
					[
